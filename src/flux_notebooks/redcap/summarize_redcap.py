# src/flux_notebooks/redcap/summarize_redcap.py (simplified)
...
def summarize_redcap(folder: Path) -> Dict[str, Dict]:
    df = load_site_csvs(folder)

    # Use only baseline rows
    baseline = df[df.get("redcap_event_name") == "t1_arm_1"].copy()
    baseline["age_years"] = baseline.get("youth_age_y", np.nan) + baseline.get("youth_age_m", 0)/12
    baseline["age_block"] = baseline["age_years"].apply(to_age_block)

    # Sex mapping (simplified)
    sex_map = {1:"Male",2:"Female"}
    if "sex" in baseline.columns:
        baseline["sex_lbl"] = baseline["sex"].map(sex_map)
    else:
        baseline["sex_lbl"] = "NA"
    ...
    # Age blocks
    age_counts = baseline.dropna(subset=["age_block"]).groupby(["site","age_block"])["record_id"].nunique().reset_index(name="Count")
    figs["age"] = px.bar(age_counts, x="age_block", y="Count", color="site",
                         category_orders={"site": SITE_ORDER}, color_discrete_map=SITE_COLOURS,
                         title="Age groups by site (baseline)")

    # Sex
    sex_counts = baseline.groupby(["site","sex_lbl"])["record_id"].nunique().reset_index(name="Count")
    figs["sex"] = px.bar(sex_counts, x="sex_lbl", y="Count", color="site",
                         category_orders={"site": SITE_ORDER}, color_discrete_map=SITE_COLOURS,
                         title="Sex distribution by site")

    # Site totals
    totals = baseline.groupby("site")["record_id"].nunique().reset_index(name="N")
    figs["totals"] = px.bar(totals, x="site", y="N", color="site",
                            category_orders={"site": SITE_ORDER}, color_discrete_map=SITE_COLOURS,
                            title="Total participants per site")

    return {"tables": {"baseline": baseline}, "figures": figs}

...
def _filter_and_tabulate(paths: list[str], base: Path, layout, cols: list[str],
                         contains: str, limit: int) -> pd.DataFrame:
    if contains:
...
def bids_explorer(dataset_root: Path):
    ...
    txt_w = widgets.Text(value='', description='contains', placeholder='e.g. T1w.json',
                         layout=Layout(width=W_SHORT), style={'description_width': DESC_W},
    ...
    cols_w  = widgets.SelectMultiple(options=['subject','session','task','run','datatype','suffix'],
                                     ...
                                     description='cols', layout=Layout(width=W_SHORT),
                                     style={'description_width': DESC_W})
    ...
    def run_search(_=None):
        with out_area:
            ...
            df = _filter_and_tabulate(paths, base, layout, list(cols_w.value), txt_w.value, int(limit_w.value))
            if df.empty:
    ...
    ], layout=Layout(grid_template_columns='repeat(3, 260px)', grid_gap='10px 20px',
...
__all__ = [
    ...
    # The following are useful for unit tests; keep or remove as you like.
